...
const ImageSlider = () => {
  ...
  return (
    <div name='foods' className='max-w-[1300px] h-[320px] md:h-[580px] w-full m-auto my-16 relative group'>
      <div
        className='w-full h-full rounded-2xl bg-center bg-cover duration-500'
        style={{ backgroundImage: `url(${slides[currentIndex].url})` }}
      ></div>
      {/* Left Arrow */}
      <div className='absolute top-[50%] -translate-x-0 translate-y-[-50%] left-11 text-2xl rounded-full p-2 group-hover:bg-black/20 text-white cursor-pointer'>
        <BsChevronCompactLeft onClick={prevSlide} size={30} />
      </div>
      {/* Right Arrow */}
      <div className='absolute top-[50%] -translate-x-0 translate-y-[-50%] right-11 text-2xl rounded-full p-2 group-hover:bg-black/20 text-white cursor-pointer'>
